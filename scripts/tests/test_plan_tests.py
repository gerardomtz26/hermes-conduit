...
class BisectPartitionTests(unittest.TestCase):
    """Pins the frozen bisect table against the exact unit-5 membership it
    splits (PR #180 run 35047088960 plan artifact, stored LPT order):
    mechanical 14/13 split, disjoint, exact union, every other lane
    unchanged, and every listed class discoverable."""
    ...
    def test_halves_partition_old_unit5_exactly(self):
        lanes = self._lanes()
        a = lanes["unit-5a"]
        b = lanes["unit-5b"]
        # Mechanical split: stored order, first 14 / remaining 13.
        self.assertEqual(a, self.OLD_UNIT5[:14])
        self.assertEqual(b, self.OLD_UNIT5[14:])
        self.assertFalse(set(a) & set(b), "the bisect halves overlap")
        self.assertEqual(set(a) | set(b), set(self.OLD_UNIT5),
                         "the bisect union must equal the old unit-5 exactly")
        self.assertEqual(len(a) + len(b), len(self.OLD_UNIT5))
